...
vec4 computeLight()
{
    ...
    for(int fuente = 0; fuente < 6; fuente++) {
        vec4 lightDir = normalize(matriz_vista * gl_LightSource[fuente].position - posicion_);
        vec4 ambient = ka * gl_LightSource[fuente].ambient;

        float angle = acos(dot(-lightDir, normalize(matriz_vista * vec4(gl_LightSource[fuente].spotDirection,0.0))));
        float cutoff = radians(clamp(gl_LightSource[fuente].spotCutoff, 0.0, 90.0));
   
        if(angle < cutoff) {
      
            float spotFactor = pow(dot(-lightDir, normalize(matriz_vista * vec4(gl_LightSource[fuente].spotDirection,0.0))), 10.0);//gl_LightSource[fuente].spotExponent);
